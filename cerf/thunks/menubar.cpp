...
LRESULT CALLBACK Win32Thunks::MenuBarWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_NCCREATE: {
        ...
        LOG(API, "[API] Menu WM_NCCREATE: lpCreateParams=0x%08X\n", mci_addr);
        if (mci_addr && s_instance) {
            ...
            LOG(API, "[API] Menu MCI: hinst=0x%08X, menuName=0x%08X, notify=0x%08X\n",
            ...
            if (hinst == 0 && menu_name != 0) {
                ...
                if (native_mod && menu_name) {
                    ...
                    LOG(API, "[API] Menu WM_NCCREATE: LoadMenuW(0x%08X, %d) -> 0x%p (err=%d)\n",
                        hinst, menu_name, hMenu, hMenu ? 0 : GetLastError());
                } else if (!native_mod && menu_name) {
                    LOG(API, "[API] Menu WM_NCCREATE: no native module for hinst=0x%08X\n", hinst);
                }
            }
    ...
    case WM_CREATE: {
        ...
        if (hMenu) {
            ...
            ReleaseDC(hwnd, hdc);
            LOG(API, "[API] Menu WM_CREATE: %d items, total_width=%d\n", count, total_width);
            if (total_width > 0) {
                CREATESTRUCTW* cs = (CREATESTRUCTW*)lParam;
    ...
    case WM_SIZE: {
        ...
        LOG(API, "[API] Menu WM_SIZE: type=%d new_client=%dx%d window=%dx%d\n",
