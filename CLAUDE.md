...
cerf/
  ...
  log.h / log.cpp                 - Categorized logging (API, PE, EMU, TRACE, CPU, REG, DBG, VFS)
  cpu/
...
- `LOG()` macro for categorized output: `LOG(API, ...)`, `LOG(PE, ...)`, `LOG(EMU, ...)`, etc.
- Categories: API, PE, EMU, TRACE, CPU, REG, DBG, VFS (defined in `log.h`)
